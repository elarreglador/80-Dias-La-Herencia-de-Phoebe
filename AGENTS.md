# AGENTS.md — PF2026 (Phileas Fogg 2026)
...
- **Regla del Este**: solo destinos con `destination.lng > current.lng`. Se aplica por omisión filtrando el catálogo; nunca renderizar botón hacia el oeste. Sin validación punitiva.
- **Sin avión**: `TransportMode.flight` no existe y no debe crearse.
- **Ciclo Fogg**: `06:00 local` carta → `07:00` ventana elección → `08:00-22:00` avance minuto a minuto → `22:00` carta+descanso `5-8h`. `localTime = utc + lng/15`. `sleeper` (train/ship) permite avanzar de noche.
