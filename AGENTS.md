...
- **Ciclo Fogg**: en 2026 protagonizado por **Phoebe Fogg** (heredera del ciclo original de Phileas, 1872). `06:00 local` carta → `07:00` ventana elección → `08:00-22:00` avance minuto a minuto → `22:00` carta+descanso `5-8h`. `localTime = utc + offsetIANA(utc, city.timezone)` — hora civil DST-correcta vía `TimezoneService` (IANA, formato `-07:00/+02:00` derivado, no persistido). `sleeper` (train/ship) permite avanzar de noche. El apellido `Fogg` se mantiene como término ubicuo (`foggPosition`, `Ciclo Fogg`) — no usar `Traveler`. Históricamente `lng/15` era tiempo solar medio, hoy sustituido por civil con `package:timezone`.
